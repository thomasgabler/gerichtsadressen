...
class FileDownloader:
    ...
    def download_new_file(self, link, filepath, session):
        ...
        if session is None:
            try:
                request = requests.get(link, headers=self.headers, timeout=220, stream=True,verify=True)
    ...
    def continue_file_download(self, link, filepath, current_bytes, total_bytes):
        ...
        try:
            request = requests.get(link, headers=range_header, timeout=130, stream=True)
